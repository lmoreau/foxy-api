...
const httpTrigger = async function (context, req) {
    ...
    try {
        ...
        const azurePlacesResponse = await axios_1.default.get(`${baseUrl}/api/GetAzurePlaces?lat=${foxy_latitude}&lon=${foxy_longitude}&code=${azurePlacesFunctionKey}`);
        let azurePlaces = azurePlacesResponse.data.businesses; // Correct path for Azure Places
        // Call GetGooglePlaces with the lat/lon, including the function key
        const googlePlacesResponse = await axios_1.default.get(`${baseUrl}/api/GetGooglePlaces?latitude=${foxy_latitude}&longitude=${foxy_longitude}&code=${googlePlacesFunctionKey}`);
        let googlePlaces = googlePlacesResponse.data.places; // Correct path for Google Places
        ...
        try {
            ...
            const response = await axios_1.default.post(`${baseUrl}/api/OpenAIStructuredResponse?code=${openAIStructuredResponseFunctionKey}`, {
                buildingId,
                foxy_fulladdress,
                combinedPlaces
            });
...
function combinePlaces(azurePlaces, googlePlaces, relatedAccounts) {
    const combined = [];
    ...
    googlePlaces.forEach(googlePlace => {
        ...
        if (googlePlace.phone_number) {
            if (azureMap.has(googlePlace.phone_number)) {
                ...
                const matchingAccount = relatedAccounts.find(acc => (acc.phone && acc.phone === googlePlace.phone_number) ||
    ...
    azureMap.forEach(place => {
        const matchingAccount = relatedAccounts.find(acc => (acc.phone && acc.phone === place.phone_number) ||
...
function extractAndValidateBuildingId(context, req) {
    let buildingId = ((req.query.id || (req.body && req.body.id)) || '').trim();
...
async function queryRelatedAccounts(context, buildingId, accessToken) {
    ...
    try {
        ...
        return response.data.value.map((accountLocation) => ({
...
async function queryDataverseBuilding(context, buildingId, accessToken) {
    const dataverseUrl = process.env.DATAVERSE_URL;
    const sanitizedDataverseUrl = dataverseUrl.endsWith('/') ? dataverseUrl.slice(0, -1) : dataverseUrl;
